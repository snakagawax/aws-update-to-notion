...
def translate_text(text):
    try:
        ...
        log_debug("Error translating text", error=str(e))
        return text
...
def generate_summary(text):
    try:
        ...
        log_debug("Error generating summary", error=str(e))
...
def add_to_notion(item, tags, article_content):
    ...
    try:
        log_debug("Adding article to Notion", article_title=item['title'], tags=tags)
        response = requests.post(NOTION_API_URL, headers=headers, json=data)
        response.raise_for_status()
        notion_page_id = response.json()["id"]
        log_debug("Article added to Notion successfully", notion_page_id=notion_page_id, article_title=item['title'])
        return notion_page_id
    except requests.exceptions.RequestException as e:
        log_debug("Error adding to Notion", error=str(e), status_code=e.response.status_code, content=e.response.content)
        return None
